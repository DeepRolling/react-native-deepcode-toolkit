...
export function transDataToTimeStamp(date: string, dateFormat: string, spilt: string[], regex?: RegExp) {
    let dateSplitArray;
    if (regex !== undefined) {
    ...
    //correct mouth value
    let mouthIndex = dateFormatSplitArray.findIndex(value => value.includes('M'));
 ...
 * @param separator
 */
function splitString(targetString: string, initialArray: string[], separator: string[]) {
...
const curriedDateformat = R.curry<(fmt: string, date: Date) => string>(dateFormat);
 ...
 * format date with YYYY-MM-dd HH:mm
 */
export const dateFormatWithYYYYMMddHHmmSS = curriedDateformat('YYYY-MM-dd HH:mm:SS');
/**
 * format date with YYYY-MM-dd+HH:mm
 */
export const dateFormatWithYYYYMMddPlusHHmm = curriedDateformat('YYYY-MM-dd+HH:mm');
/**
...
export function timeDeal(timeStr: string, timeLength?: number) {
    ...
    try {
        ...
        timeList[1] = timeList[1].slice(0, timeLength !== undefined ? timeLength : 5);
    } catch {}
    return timeList;
}
 ...
 */
export function calculateDate(dateParameter: string, calValue: number, disableZeroAppend?: boolean) {
    //返回日期的毫秒值
...
export function calculateMonth(monthParameter: string, calValue: number) {
    ...
    //should reduce year
    year = plusOp ? year + (notReachAYear ? 0 : howManyYear) : notReachAYear ? year - (mouth + calValue <= 0 ? 1 : 0) : year - howManyYear;
    //calculate month
    let rightMonth = 0;
    if (mouth + calValue < 0) {
        rightMonth = plusOp ? mouth + (Math.abs(calValue) % 12) : mouth - (Math.abs(calValue) % 12);
        if (rightMonth < 0) {
