...
export function calculateWrapItemWidth(countOfEachRow: number, itemMargin: number, paddingHorizontal: number) {
    return deviceWidthDp - paddingHorizontal - ((countOfEachRow - 1) * itemMargin) / countOfEachRow;
}

export function calculateDeviceWidthAndHeight() {
    console.log('screen Dimensions(calculateDeviceWidthAndHeight) : ' + Dimensions.get('window').width + ' : ' + Dimensions.get('window').height);
...
function getRealFontsize(number: any) {
    number = Math.round(
        ((number * Math.min(deviceHeightDp / uiHeightPx, deviceWidthDp / uiWidthPx) + 0.5) * PixelRatio.get()) / PixelRatio.getFontScale(),
    );
    return number / PixelRatio.get();
}

export {getRealWidth, getRealHeight, getRealFontsize, deviceWidthDp, deviceHeightDp};
